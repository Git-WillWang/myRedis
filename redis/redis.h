...
#define REDIS_ERR -1
/*默认的服务器配置值*/
#define REDIS_DEFAULT_HZ 10 //时间中断频率
...
#define REDIS_LRU_CLOCK_MAX ((1<<REDIS_LRU_BITS)-1) //obj->lru的最大值
#define REDIS_LRU_CLOCK_RESOLUTION 1000 
typedef struct redisObject {
	unsigned type : 4;
	unsigned encoding : 4;
	unsigned lru : REDIS_LRU_BITS;//与server.lruclock的时间差值
	int refcount;//引用计数算法使用的引用计数器
	void *ptr;//指向实际值的指针
}robj;

#define LRU_CLOCK() ((1000/server.hz<=REDIS_LRUCLOCK_RESOLUTION)? server.lruclock:getLRUClock())
...
typedef struct redisDb {
	...
	dict *dict;
	//键的过期时间，字典的键为键，字典的值为过期事件UNIX时间戳
	dict *expires;
	//正处于阻塞状态的键
	dict *blocking_keys;
	//可以接触阻塞的键
...
typedef struct blockingState {
	mstime_t timeout;//阻塞时限，如果当前UNIX时间>timeout，则操作超时
	//阻塞的键
	dict *keys;
...
/*
记录订阅模式的结构
Pub/Sub功能（means Publish, Subscribe）即发布及订阅功能。基于事件的系统中，
Pub/Sub是目前广泛使用的通信模型，它采用事件作为基本的通信机制，提供大规模系统
所要求的松散耦合的交互模式：订阅者(如客户端)以事件订阅的方式表达出它有兴趣接收
的一个事件或一类事件；发布者(如服务器)可将订阅者感兴趣的事件随时通知相关订阅者。
*/
...
typedef struct {
	robj *subject;//列表对象
	unsigned char encoding;//对象所用编码
	unsigned char direction;//迭代方向
	unsigned char *zi;// ziplist 索引，迭代 ziplist 编码的列表时使用
	listNode *ln;// 链表节点的指针，迭代双端链表编码的列表时使用
}listTypeIterator;
...
//表示开区间/闭区间范围的结构 用于分数比对
typedef struct {
	double min, max;
	int minex, maxex;
}zrangespec;
//表示开区间/闭区间范围的结构 用于字典序比对
typedef struct {
	robj *min, *max;
	int minex, maxex;
}zlexrangespec;
